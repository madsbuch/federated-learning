...
export let newestLayer: ModelLayersStore = modelLayersStore;
...
const enroll = (publicKey: string, socket: ServerWebSocket<any>): string => {
  const existingParty = Object.keys(parties).find(
    (partyId) => parties[partyId].participants.length < 3,
  );
...
export const makeWsServer = (port = 4000) =>
  Bun.serve({
      ...
      async message(ws, message) {
        try {
          ...
          if (m.methodName === "getModel") {
            ...
            const weightShapes = modelLayersStore.model
              .getWeights()
              .map((w) => w.shape);
            ...
            Object.values(parties).forEach((party) => {
              const allPartialSumsAreIn = !party.participants.find(
                (p) => p.partialSum === undefined,
              );

              if (allPartialSumsAreIn && !party.sum) {
                const sumOfPartials = party.participants
                  .map((p) => ({
                    ...p.partialSum,
                    modelWeights: p.partialSum?.modelWeights.map((mw, i) =>
                  ...
                  .reduce((p, participant) => {
                    ...
                    return {
                      trainedBatches:
                        p.trainedBatches + participant.trainedBatches,
                ...
                const ws = sumOfPartials.modelWeights?.map((t, i) =>
                  t.div(3).add(currentTensors[i]),
                );
                if (
                  !ws ||
                  !sumOfPartials.modelWeights ||
                  !sumOfPartials.trainedBatches
                ) {
                  throw new Error("Could not calculate new weights");
                }
                newModel.setWeights(ws);

                const newLayer: ModelLayersStore = {
                  id: randomUUID(),
                  model: newModel,
                  children: [],
                  time: new Date(),
                  sumOfPartials: {
                    trainedBatches: sumOfPartials.trainedBatches!,
                    modelWeights: sumOfPartials.modelWeights!,
                  },
                };
