...
import { addListOfTensors, assembleTensors, combineInts, splitInt, splitTensors, subListOfTensors, weightAsArray } from "../src/mpc-sum-sharing";
...
test("Can add list of tensors", async () => {
  ...
  expect(await Promise.all(t1.map(async e => [...await e.data()]))).toEqual([[1, 1], [2, 2]])
  expect(await Promise.all(result.map(async e => [...await e.data()]))).toEqual([[2, 2], [4, 4]])
})
...
test("Can sub list of tensors", async () => {
  ...
  expect(await Promise.all(t1.map(async e => [...await e.data()]))).toEqual([[1, 1], [2, 2]])
  expect(await Promise.all(result.map(async e => [...await e.data()]))).toEqual([[0, 0], [0, 0]])
})
...
test("Can split and combine models", async () => {
  ...
  const normalized1 = (model1.getWeights()).map(async t => [...await t.mul(1000000).floor().div(1000000).data()])
  const normalized2 = (model2.getWeights()).map(async t => [...await t.mul(1000000).floor().div(1000000).data()])
