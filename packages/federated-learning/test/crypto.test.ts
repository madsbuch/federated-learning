...
test("Encryption / Decryption is isomorph", async () => {
  ...
  expect(await decryptAfterDecode(keyPair, await encryptAndEncode(publicKeyEncoded, "42"))).toEqual("42")
});
...
test("Encrypts and decrypts tensors", async () => {
  ...
  const recovered = await decryptAfterDecode(keyPair, await encryptAndEncode(publicKeyEncoded, obj))
  ...
  expect(JSON.parse(recovered)).toEqual({ "batches": 42, "weights": { "0": 11, "1": 23, "2": 33 } })
})
...
test("Can recover an encrypted model after decryption", async () => {
  ...
  const tensors = returnObj.weights.weightData.map((data: number[], index: number) => {
