...
export const modelShapes = createConvModel()
  .getWeights()
  .map((w) => w.shape);
    ...
    callbacks: {
      onBatchEnd: async (batch, logs) => {
        ...
        if (logs) {
          ...
          if (cb.plotAccuracy)
            cb.plotAccuracy(trainBatchCount, logs.acc, "train");
        }
      ...
      onEpochEnd: async (epoch, logs) => {
        if (logs) {
          valAcc = logs.val_acc;
          if (cb.plotLoss)
            cb.plotLoss(trainBatchCount, logs.val_loss, "validation");
          if (cb.plotAccuracy)
            cb.plotAccuracy(trainBatchCount, logs.val_acc, "validation");
        }
