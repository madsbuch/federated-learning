...
import { algorithm, decryptAfterDecode, encodePublicKey, encryptAndEncode } from './crypto';
...
export class PrivateSum {
    ...
    private privatePart?: PrivatePart
    ...
    constructor(private cb: PrivateSumCallbacks, private url: string) { }
    ...
    async start(privateNumber: PrivatePart) {
        this.privatePart = privateNumber

        if (this?.partySize && this.privatePart && this.friends.length === this.partySize - 1) {
            const batchPieces = splitInt(this.privatePart.trainedBatches, this.partySize)
            const weightPieces = splitTensors(this.privatePart.modelWeights, this.partySize)
            ...
            // We just let this happen (ie. it is intentional that we do not await)
            await Promise.all(this.friends.map(async (friend, i) => {
                const piece: SumObj = {
                    trainedBatches: this.piecesToShare![i].trainedBatches,
                    modelWeights: await Promise.all(this.piecesToShare![i].modelWeights.map(async mw => [...await mw.data()]))
                }
                this.socket?.send(prepMsg({
                    ...
                    // Encrypted
                    piece: JSON.stringify(await encryptAndEncode(friend.publicKey, JSON.stringify(piece)))
                }))
            }))
        } else {
            return false
    ...
    async onMessage(m: WsMsg) {
        if (m.methodName === "configuration") {
            ...
            if (friend && this.keyPair) {
                // Decrypt first
                const json = JSON.parse(await decryptAfterDecode(this.keyPair, JSON.parse(m.piece)))
                const piece = SumObj.parse(json)

                friend.piece = {
                    trainedBatches: piece.trainedBatches,
                    modelWeights: piece.modelWeights.map(mw => tf.tensor(mw))
                }
            }
            ...
            if (!this.friends.find(f => f.piece === undefined)) {
                // Sum everything up!
                const totalPartialSum = this.friends.map(e => e.piece).reduce((prev, cur) => {
                    return {
                        trainedBatches: prev?.trainedBatches! + cur?.trainedBatches!,
                        modelWeights: addListOfTensors(cur?.modelWeights!, prev?.modelWeights!)
                    }
                })

                // And send it to the server
                new Promise(async () => {
                    const promises = totalPartialSum?.modelWeights.map(async (mw) => Array.from(await mw.data()))
                    if (!promises) {
 ...
 * @returns 
 */
export const splitTensors = (weights: tf.Tensor<tf.Rank>[], n: number): tf.Tensor<tf.Rank>[][] => {
    ...
    const risidual = subListOfTensors(weights, randomPieces.reduce((p, t) => addListOfTensors(p, t)));
...
export const splitInt = (x: number, n: number): number[] => {
    const randomPieces = ([...new Array(n - 1)]).map(_s => Math.floor(Math.random() * 1000) + 1)
...
// Util
export const addListOfTensors = (t1: tf.Tensor[], t2: tf.Tensor[]): tf.Tensor[] => {
    return t1.map((t11, i) => t11.add(t2[i]))
}
export const subListOfTensors = (t1: tf.Tensor[], t2: tf.Tensor[]): tf.Tensor[] => {
    return t1.map((t11, i) => t11.sub(t2[i]))
}
export const weightAsArray = (model: tf.LayersModel) => Promise.all(model.getWeights().map(async (w) => Array.from(await w.data())))