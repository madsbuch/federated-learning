 ...
 * @returns
 */
export const encryptAndEncode = async (
  publicKeyEncoded: string,
  element: string,
): Promise<EncrypedPackage> => {
  const publicKeyDecoded = await crypto.subtle.importKey(
    "jwk",
    JSON.parse(atob(publicKeyEncoded)),
    algorithm,
    true,
    ["encrypt"],
  );
  ...
  // Make an AES key
  const messageKey = await crypto.subtle.generateKey(aesAlgorithm(iv), true, [
    "encrypt",
    "decrypt",
  ]);
  ...
  const encrypedMessage = await crypto.subtle.encrypt(
    aesAlgorithm(iv),
    messageKey,
    writeBuffer,
  );
...
export const decryptAfterDecode = async (
  { privateKey }: CryptoKeyPair,
  encodedAndEncrypted: EncrypedPackage,
): Promise<string> => {
